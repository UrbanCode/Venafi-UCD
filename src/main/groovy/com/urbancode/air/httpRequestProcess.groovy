...
class httpRequestProcess {
	...
	public String httpGetSystemProperties() {
		def allowedAgents = ""
		String getVersionOfSystemPropsURL = serverConnection.getWebURL() + "/property/propSheet/system%26properties.1"
		String versionOfSystemPropsResponse = httpGet(getVersionOfSystemPropsURL)
		// println(versionOfSystemPropsResponse)
		JSONObject versionOfSystemPropsResponseJSON = new JSONObject(versionOfSystemPropsResponse)

		println(versionOfSystemPropsResponseJSON.versionCount)

		String getSystemPropsURL = serverConnection.getWebURL() + "/property/propSheet/system%26properties." + versionOfSystemPropsResponseJSON.versionCount

		String systemPropsResponse = httpGet(getSystemPropsURL)

		def systemPropsSlurper = new JsonSlurper()
		def systemPropsJSONObject = systemPropsSlurper.parseText(systemPropsResponse.toString())

		println(systemPropsJSONObject.properties.size())

		//def systemProps = systemPropsSlurper.parseText(systemPropsJSONObject.properties)

		println("-----------------------------------------------------")

		for (def counter = 0; counter < systemPropsJSONObject.properties.size(); counter++) {
			def name = systemPropsJSONObject.properties[counter].name
			def value = systemPropsJSONObject.properties[counter].value
		...
		//if (SystemPropsJSONObject.has("Venafi-agents : Non-Production Environment")) {
		//	allowedAgents = systemProps["Venafi-agents : Non-Production Environment"]

		//}

		//println(allowedAgents)
	}
